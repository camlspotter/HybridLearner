...
#include <bits/stdc++.h>	//used for assert with count in: assert(locations.count(Loc_Id)!=0);
#include <assert.h>
...
hybridAutomata::hybridAutomata(std::map<int, location::ptr>& list_locs, location::ptr init_loc,
		int dim) {
	locations = list_locs;	//assigning a map to another map
