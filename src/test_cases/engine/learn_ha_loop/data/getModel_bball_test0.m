...
set_param(bdroot, 'SolverType','Variable-step', 'StopTime', 'timeFinal', 'SolverName', 'ode15s', 'MaxStep', 'timeStepMax'); 
ch.ActionLanguage = 'C'; 
...
str = ['loc1', 10, ... 
 'du: ', 10, ... 
 '    x1_dot = -0.0000000 * x0 + -0.0000000 * x1 + 1.0000000 * x2 + -0.0000000 * 1;', 10, ...  
 '    x2_dot = 0.9999996 * x0 + -0.0000000 * x1 + 0.0000000 * x2 + -0.0000035 * 1;', 10, ...  
 'exit: ', 10, ... 
 '    x1_out = x1;', 10, ...  
 '    x2_out = x2;', 10, ...  
 ]; 
...
cb1_0.LabelString = '[-0.00571861051106969 * x0 + 3.890363173548988 * x1 + 0.004039401282109726 * x2 + -0.784232895697025 * 1 <= 0.000000 + 0]{x1 = 0.0000000 * x0 + 1.0000000 * x1 + 0.0000000 * x2 + 0.000; x2 = 0.1679569 * x0 + 0.6178552 * x1 + -0.7958158 * x2 + 1.094; }'; 
...
chartOutSignal = get_param('bball_test0/Chart', 'PortHandles'); 



 %% *** Adding Input  components **** 
add_block('simulink/Sources/In1', 'bball_test0/x0In'); 
x0Input = get_param('bball_test0/x0In', 'PortHandles'); 
set_param('bball_test0/x0In', 'Port', '1'); 
set_param('bball_test0/x0In', 'SignalType', 'auto'); 
set_param('bball_test0/x0In', 'position', [-40, 18, 0, 33]); 

 
add_line('bball_test0', x0Input.Outport(1), chartOutSignal.Inport(1)); 
...
add_block('simulink/Sinks/Out1', 'bball_test0/x1Out'); 
x1Output = get_param('bball_test0/x1Out', 'PortHandles'); 
set_param('bball_test0/x1Out', 'SignalName', 'x1Out'); 
set_param('bball_test0/x1Out', 'Port', '1'); 
set_param('bball_test0/x1Out', 'SignalType', 'auto'); 
set_param('bball_test0/x1Out', 'position', [140, 18, 180, 33]); 

 
add_line('bball_test0', chartOutSignal.Outport(1), x1Output.Inport(1)); 

 

add_block('simulink/Sinks/Out1', 'bball_test0/x2Out'); 
x2Output = get_param('bball_test0/x2Out', 'PortHandles'); 
set_param('bball_test0/x2Out', 'SignalName', 'x2Out'); 
set_param('bball_test0/x2Out', 'Port', '2'); 
set_param('bball_test0/x2Out', 'SignalType', 'auto'); 
set_param('bball_test0/x2Out', 'position', [140, 58, 180, 73]); 

 
add_line('bball_test0', chartOutSignal.Outport(2), x2Output.Inport(1)); 

 

add_block('simulink/Sinks/Out1', 'bball_test0/x0Out'); 
x0Output = get_param('bball_test0/x0Out', 'PortHandles'); 
set_param('bball_test0/x0Out', 'SignalName', 'x0Out'); 
set_param('bball_test0/x0Out', 'Port', '3'); 
set_param('bball_test0/x0Out', 'SignalType', 'auto'); 
set_param('bball_test0/x0Out', 'position', [140, 98, 180, 113]); 
add_line('bball_test0', x0Input.Outport(1), x0Output.Inport(1)); 

 



Simulink.BlockDiagram.arrangeSystem('bball_test0'); 
Simulink.BlockDiagram.arrangeSystem('bball_test0/Chart'); 
