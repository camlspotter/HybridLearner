/*!
 * This file is also used in XSpeed to compute exp(Matrix).
 *
 *   \file expm.hpp
 *
...
#include <boost/numeric/ublas/vector_proxy.hpp> // for avoiding bug in lu.hpp
#include <boost/numeric/ublas/triangular.hpp> // for avoiding bug in lu.hpp

namespace boost {
namespace numeric {
namespace ublas {

template<typename MATRIX> MATRIX expm_pad(const MATRIX &H,
		typename type_traits<typename MATRIX::value_type>::real_type t = 1.0,
		const int p = 6) {
